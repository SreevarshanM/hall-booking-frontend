...
function StudentDashboardPendingRequests(props) {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8800/api/booking/userBookings?studentid=${userData.Student_ID}`)
        .then((response) => {
    ...
    const filteredBookings = selectedStatus === 'all' ? bookingData : bookingData.filter(booking => booking.Status === selectedStatus);
    ...
    const options = { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }; //DATE OPTIONS
       ...
       <div className="bg-neutral-100 w-full">
             <nav className="bg-white border-gray-200">
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    ...
                    <div className="mt-2 lg:mt-0 w-full md:block md:w-auto" id="navbar-default">
                        <select id="email" value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="bg-[#f8fafa] border border-gray-300 text-gray-900 text-md rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5" required>
            ...
            <div className="p-4 sm:p-10 max-h-[550px] overflow-y-auto">
                ...
                {filteredBookings.map((booking) => (
                    <li className="p-2">
                        <div className={`${getStatusClassName(booking.Status)}`} onClick={() => handleDivClick(booking.status, booking._id)}>
                            <h5 className="mb-2 text-xl font-bold tracking-tight">{booking.Hall_Name} | {new Date(booking.Date).toLocaleDateString('en-US', options)} | {new Date(booking.Time_From).toLocaleTimeString('en-US', timeOptions)} TO {new Date(booking.Time_From).toLocaleTimeString('en-US', timeOptions)} </h5>
